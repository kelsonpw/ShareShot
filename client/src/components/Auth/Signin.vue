    ...
    <v-layout row wrap>
      <v-flex xs12 sm6 offset-sm3>
        <v-card color="secondary" dark>
          <v-container>
            <v-form v-model="isFormValid" lazy-validation @submit.prevent="handleSigninUser" ref="form">
...
export default {
  ...
  data() {
    return {
      ...
      passwordRules: [
        ...
        // make sure password is atleast 7 chars
        password =>
          password.length > 6 || 'Password must be atleast 7 characters'
      ]
    };
  },
  computed: {
  ...
  watch: {
    user(value) {
      // if user value changes from null to object, redirect to home

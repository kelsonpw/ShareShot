        ...
        <v-card hover>
          <v-card-title>
            ...
            <v-btn v-if="user" large icon>
              <v-icon color="grey" large>favorite</v-icon>
            </v-btn>
            <h3 class="ml-3 font-weight-thin">{{getPost.likes}} LIKES</h3>
            <v-spacer></v-spacer>
            <v-icon color="info" large @click="goToPreviousPage">arrow_back</v-icon>
          </v-card-title>
      ...
      <v-layout class="mb-3" v-if="user">
        <v-flex xs12>
          <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleAddPostMessage">
            <v-layout row>
              <v-flex xs12>
                <v-text-field v-model="messageBody" :rules="messageRules" clearable :append-outer-icon="messageBody && 'send'" label="Add Message" type="text" @click:append-outer="handleAddPostMessage" prepend-icon="email" required>
                </v-text-field>
              </v-flex>
            </v-layout>
          </v-form>
            ...
            <template v-for="message in getPost.messages">
              ...
              <v-list-tile :key="message.title" avatar inset>
                ...
                <v-list-tile-action class="hidden-xs-only">
                  <v-icon :color="checkIfOwnMessage(message) ? 'primary' : 'secondary'">chat_bubble</v-icon>
                </v-list-tile-action>
              </v-list-tile>
            </template>
          </v-list>
  ...
  data() {
    return {
      ...
      messageRules: [
        ...
        message =>
          message.length < 76 || 'Message must be less than 75 characters'
      ]
    };
  },
  apollo: {
